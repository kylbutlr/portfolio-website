import React from 'react';

export default () => (
  <div className='bio'>
    <p>
      I specialize in using <span className='bold'>React</span>,{' '}
      <span className='bold'>JavaScript (ES6)</span>, <span className='bold'>CSS/HTML</span>, and{' '}
      <span className='bold'>Node</span> to build anything from responsive small business and
      personal websites to rich interactive web applications.
    </p>
    <p>
      I have experience working with <span className='bold'>PostgreSQL</span>,{' '}
      <span className='bold'>Express</span>, <span className='bold'>Axios</span>,{' '}
      <span className='bold'>NPM</span>, <span className='bold'>Jest</span>,{' '}
      <span className='bold'>Sass</span>, and <span className='bold'>Bulma</span>, as well as
      previously used <span className='bold'>Java</span>, <span className='bold'>PHP</span>, and{' '}
      <span className='bold'>Python</span>. I look forward to learning and utilizing many more
      languages and frameworks throughout my career.
    </p>
  </div>
);
